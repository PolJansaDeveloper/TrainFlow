...
enum class TrackingType(val key: String) {
    ...
    companion object {
        fun fromKey(key: String): TrackingType = entries.firstOrNull { it.key == key } ?: RepsOnly
    }
}
...
data class WeeklyPlan(
    ...
    val days: List<DayWorkout> = (1..7).map { DayWorkout(dayOfWeek = it, workoutName = "", isRestDay = true) }
)
